...
print('\nInitial model built & MRSA completed.')
print('======================================================')
...
base_shearX = modal_combo(np.loadtxt('./mrsa_results/dirX/baseShearX.txt'), lambda_list, damping_ratio, num_modes).sum()

base_shearY = modal_combo(np.loadtxt('./mrsa_results/dirY/baseShearY.txt'), lambda_list, damping_ratio, num_modes).sum()
...
elf_base_shear = nz_horiz_seismic_shear(spectral_shape_factor, hazard_factor, return_per_factor_sls,
                           return_per_factor_uls, fault_factor, perform_factor, ductility_factor,
                           seismic_weight)

elf_force_distrib = nz_horiz_force_distribution(elf_base_shear, story_weights, np.cumsum(story_heights))


floor_dimen_x = 29.410 * m
floor_dimen_y = 31.025 * m

eccentricity_x = 0.05 * floor_dimen_x
eccentricity_y = 0.05 * floor_dimen_y

torsional_mom_x = elf_force_distrib * eccentricity_x
torsional_mom_y = elf_force_distrib * eccentricity_y
