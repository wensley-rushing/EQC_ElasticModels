...
def create_floor(elev, floor_num, floor_label=''):
    ...
    if floor_num != '00':
        ...
        building_nodes.append(floor_node_tags)
...
# ============================================================================
# Create mass matrix for each DOF
# ============================================================================
ndof = 2
mass_matrix = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_1 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_2 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_3 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_4 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_5 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_6 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_7 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_8 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_9 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
eig_vec_10 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))

for ii in range(len(story_heights)):
    node_tags = building_nodes[ii]

    for jj in range(len(node_tags)):
        node = node_tags[jj]

        mass_matrix[0][ii][jj] = ops.nodeMass(node, 1)
        mass_matrix[1][ii][jj] = ops.nodeMass(node, 2)

        eig_vec_1[0][ii][jj] = ops.nodeEigenvector(node, 1, 1)
        eig_vec_1[1][ii][jj] = ops.nodeEigenvector(node, 1, 2)

        eig_vec_2[0][ii][jj] = ops.nodeEigenvector(node, 2, 1)
        eig_vec_2[1][ii][jj] = ops.nodeEigenvector(node, 2, 2)

        eig_vec_3[0][ii][jj] = ops.nodeEigenvector(node, 3, 1)
        eig_vec_3[1][ii][jj] = ops.nodeEigenvector(node, 3, 2)

        eig_vec_4[0][ii][jj] = ops.nodeEigenvector(node, 4, 1)
        eig_vec_4[1][ii][jj] = ops.nodeEigenvector(node, 4, 2)

        eig_vec_5[0][ii][jj] = ops.nodeEigenvector(node, 5, 1)
        eig_vec_5[1][ii][jj] = ops.nodeEigenvector(node, 5, 2)

        eig_vec_6[0][ii][jj] = ops.nodeEigenvector(node, 6, 1)
        eig_vec_6[1][ii][jj] = ops.nodeEigenvector(node, 6, 2)

        eig_vec_7[0][ii][jj] = ops.nodeEigenvector(node, 7, 1)
        eig_vec_7[1][ii][jj] = ops.nodeEigenvector(node, 7, 2)

        eig_vec_8[0][ii][jj] = ops.nodeEigenvector(node, 8, 1)
        eig_vec_8[1][ii][jj] = ops.nodeEigenvector(node, 8, 2)

        eig_vec_9[0][ii][jj] = ops.nodeEigenvector(node, 9, 1)
        eig_vec_9[1][ii][jj] = ops.nodeEigenvector(node, 9, 2)

        eig_vec_10[0][ii][jj] = ops.nodeEigenvector(node, 10, 1)
        eig_vec_10[1][ii][jj] = ops.nodeEigenvector(node, 10, 2)


# Modal mass Mn
modal_mass = np.zeros((ndof, num_modes, len(building_nodes[0])))

for ii in range(len(building_nodes[0])):
    modal_mass[0][0, ii] = eig_vec_1[0][:, ii].dot(np.matmul(eig_vec_1[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][1, ii] = eig_vec_2[0][:, ii].dot(np.matmul(eig_vec_2[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][2, ii] = eig_vec_3[0][:, ii].dot(np.matmul(eig_vec_3[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][3, ii] = eig_vec_4[0][:, ii].dot(np.matmul(eig_vec_4[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][4, ii] = eig_vec_5[0][:, ii].dot(np.matmul(eig_vec_5[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][5, ii] = eig_vec_6[0][:, ii].dot(np.matmul(eig_vec_6[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][6, ii] = eig_vec_7[0][:, ii].dot(np.matmul(eig_vec_7[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][7, ii] = eig_vec_8[0][:, ii].dot(np.matmul(eig_vec_8[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][8, ii] = eig_vec_9[0][:, ii].dot(np.matmul(eig_vec_9[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    modal_mass[0][9, ii] = eig_vec_10[0][:, ii].dot(np.matmul(eig_vec_10[0][:, ii], np.diag(mass_matrix[0][:, ii])))


    modal_mass[1][0, ii] = eig_vec_1[1][:, ii].dot(np.matmul(eig_vec_1[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][1, ii] = eig_vec_2[1][:, ii].dot(np.matmul(eig_vec_2[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][2, ii] = eig_vec_3[1][:, ii].dot(np.matmul(eig_vec_3[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][3, ii] = eig_vec_4[1][:, ii].dot(np.matmul(eig_vec_4[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][4, ii] = eig_vec_5[1][:, ii].dot(np.matmul(eig_vec_5[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][5, ii] = eig_vec_6[1][:, ii].dot(np.matmul(eig_vec_6[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][6, ii] = eig_vec_7[1][:, ii].dot(np.matmul(eig_vec_7[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][7, ii] = eig_vec_8[1][:, ii].dot(np.matmul(eig_vec_8[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][8, ii] = eig_vec_9[1][:, ii].dot(np.matmul(eig_vec_9[1][:, ii], np.diag(mass_matrix[1][:, ii])))
    modal_mass[1][9, ii] = eig_vec_10[1][:, ii].dot(np.matmul(eig_vec_10[1][:, ii], np.diag(mass_matrix[1][:, ii])))

# Modal participation factor L_n
identity = np.ones(len(story_heights))
Ln = np.zeros((ndof, num_modes, len(building_nodes[0])))

for ii in range(len(building_nodes[0])):
    Ln[0][0, ii] = identity.dot(np.matmul(eig_vec_1[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][1, ii] = identity.dot(np.matmul(eig_vec_2[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][2, ii] = identity.dot(np.matmul(eig_vec_3[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][3, ii] = identity.dot(np.matmul(eig_vec_4[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][4, ii] = identity.dot(np.matmul(eig_vec_5[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][5, ii] = identity.dot(np.matmul(eig_vec_6[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][6, ii] = identity.dot(np.matmul(eig_vec_7[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][7, ii] = identity.dot(np.matmul(eig_vec_8[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][8, ii] = identity.dot(np.matmul(eig_vec_9[0][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[0][9, ii] = identity.dot(np.matmul(eig_vec_10[0][:, ii], np.diag(mass_matrix[0][:, ii])))


    Ln[1][0, ii] = identity.dot(np.matmul(eig_vec_1[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][1, ii] = identity.dot(np.matmul(eig_vec_2[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][2, ii] = identity.dot(np.matmul(eig_vec_3[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][3, ii] = identity.dot(np.matmul(eig_vec_4[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][4, ii] = identity.dot(np.matmul(eig_vec_5[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][5, ii] = identity.dot(np.matmul(eig_vec_6[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][6, ii] = identity.dot(np.matmul(eig_vec_7[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][7, ii] = identity.dot(np.matmul(eig_vec_8[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][8, ii] = identity.dot(np.matmul(eig_vec_9[1][:, ii], np.diag(mass_matrix[0][:, ii])))
    Ln[1][9, ii] = identity.dot(np.matmul(eig_vec_10[1][:, ii], np.diag(mass_matrix[0][:, ii])))


# Modal participation factor Gamma_n
gamma_n = Ln / modal_mass

# sn
sn_1 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_2 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_3 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_4 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_5 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_6 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_7 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_8 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_9 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))
sn_10 = np.zeros((ndof, len(story_heights), len(building_nodes[0])))

for ii in range(len(building_nodes[0])):
    sn_1[0][:, ii] = gamma_n[0][0][ii] * np.matmul(eig_vec_1[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_2[0][:, ii] = gamma_n[0][1][ii] * np.matmul(eig_vec_2[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_3[0][:, ii] = gamma_n[0][2][ii] * np.matmul(eig_vec_3[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_4[0][:, ii] = gamma_n[0][3][ii] * np.matmul(eig_vec_4[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_5[0][:, ii] = gamma_n[0][4][ii] * np.matmul(eig_vec_5[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_6[0][:, ii] = gamma_n[0][5][ii] * np.matmul(eig_vec_6[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_7[0][:, ii] = gamma_n[0][6][ii] * np.matmul(eig_vec_7[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_8[0][:, ii] = gamma_n[0][7][ii] * np.matmul(eig_vec_8[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_9[0][:, ii] = gamma_n[0][8][ii] * np.matmul(eig_vec_9[0][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_10[0][:, ii] = gamma_n[0][9][ii] * np.matmul(eig_vec_10[0][:, ii], np.diag(mass_matrix[0][:, ii]))

    sn_1[1][:, ii] = gamma_n[0][0][ii] * np.matmul(eig_vec_1[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_2[1][:, ii] = gamma_n[0][1][ii] * np.matmul(eig_vec_2[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_3[1][:, ii] = gamma_n[0][2][ii] * np.matmul(eig_vec_3[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_4[1][:, ii] = gamma_n[0][3][ii] * np.matmul(eig_vec_4[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_5[1][:, ii] = gamma_n[0][4][ii] * np.matmul(eig_vec_5[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_6[1][:, ii] = gamma_n[0][5][ii] * np.matmul(eig_vec_6[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_7[1][:, ii] = gamma_n[0][6][ii] * np.matmul(eig_vec_7[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_8[1][:, ii] = gamma_n[0][7][ii] * np.matmul(eig_vec_8[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_9[1][:, ii] = gamma_n[0][8][ii] * np.matmul(eig_vec_9[1][:, ii], np.diag(mass_matrix[0][:, ii]))
    sn_10[1][:, ii] = gamma_n[0][9][ii] * np.matmul(eig_vec_10[1][:, ii], np.diag(mass_matrix[0][:, ii]))
