...
from helper_functions.peak_modal_response import cqc_modal_combo
...
def create_floor(elev, floor_num, floor_label=''):
    ...
    if floor_num != '00':
        ops.node(com_node, 16.363, 14.060, elev)
...
if mrsa:
    ...
    mrsa_direc = './mrsa_results/'

    ops.recorder('Element', '-file', mrsa_direc + '/floor01_beamResp.txt',  '-region', 301, 'force')
    ...
    for ii in range (len(direcs)):
        ...
        np.savetxt(direc + 'col10_forces.txt', col_response)
...
if mrsa:
    modal_resp = np.loadtxt('./mrsa_results/dirX/col10_forces.txt')
    peak_modal_combo = cqc_modal_combo(modal_resp, lambda_list, damping_ratio, num_modes)
